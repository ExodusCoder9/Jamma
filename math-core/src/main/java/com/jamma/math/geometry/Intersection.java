...
public final class Intersection {
    ...
    public static Vector3d closestPointTriangle(Vector3d point, Vector3d v0, Vector3d v1, Vector3d v2) {
        double abx = v1.x() - v0.x(), aby = v1.y() - v0.y(), abz = v1.z() - v0.z();
        double acx = v2.x() - v0.x(), acy = v2.y() - v0.y(), acz = v2.z() - v0.z();
        double apx = point.x() - v0.x(), apy = point.y() - v0.y(), apz = point.z() - v0.z();
        double d1 = abx * apx + aby * apy + abz * apz;
        double d2 = acx * apx + acy * apy + acz * apz;
        if (d1 <= 0.0 && d2 <= 0.0) return v0;
        double bpx = point.x() - v1.x(), bpy = point.y() - v1.y(), bpz = point.z() - v1.z();
        ...
        if (vc <= 0.0 && d1 >= 0.0 && d3 <= 0.0) {
            double v = d1 / (d1 - d3);
            return new Vector3d(v0.x() + v * abx, v0.y() + v * aby, v0.z() + v * abz);
        }
        double cpx = point.x() - v2.x(), cpy = point.y() - v2.y(), cpz = point.z() - v2.z();
        ...
        if (vb <= 0.0 && d2 >= 0.0 && d6 <= 0.0) {
            double w = d2 / (d2 - d6);
            return new Vector3d(v0.x() + w * acx, v0.y() + w * acy, v0.z() + w * acz);
        }
        double va = d3 * d6 - d5 * d4;
        if (va <= 0.0 && d4 - d3 >= 0.0 && d5 - d6 >= 0.0) {
            double w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
            return new Vector3d(v1.x() + w * (v2.x() - v1.x()), v1.y() + w * (v2.y() - v1.y()), v1.z() + w * (v2.z() - v1.z()));
        }
        double denom = 1.0 / (va + vb + vc);
        double v = vb * denom;
        double w = vc * denom;
        return new Vector3d(v0.x() + v * abx + w * acx, v0.y() + v * aby + w * acy, v0.z() + v * abz + w * acz);
    }
