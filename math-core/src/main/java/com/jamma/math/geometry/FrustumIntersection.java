package com.jamma.math.geometry;

import com.jamma.math.Vector3d;
import com.jamma.math.Vector3f;
import com.jamma.math.matrix.Matrix4d;
import com.jamma.math.matrix.Matrix4f;
import java.io.Serializable;

public class FrustumIntersection implements Serializable {

    private static final long serialVersionUID = 1L;

    public static final int INSIDE = 1;
    public static final int OUTSIDE = -1;
    public static final int INTERSECT = 0;

    public static final int LEFT = 0;
    public static final int RIGHT = 1;
    public static final int BOTTOM = 2;
    public static final int TOP = 3;
    public static final int NEAR = 4;
    public static final int FAR = 5;

    public final double[][] planes;

    public FrustumIntersection() {
        planes = new double[6][4];
    }

    public FrustumIntersection(Matrix4d m) {
        planes = new double[6][4];
        set(m, false);
    }

    public FrustumIntersection(Matrix4d m, boolean vulkan) {
        planes = new double[6][4];
        set(m, vulkan);
    }

    public FrustumIntersection(Matrix4f m) {
        planes = new double[6][4];
        set(m, false);
    }

    public FrustumIntersection(Matrix4f m, boolean vulkan) {
        planes = new double[6][4];
        set(m, vulkan);
    }

    public FrustumIntersection set(Matrix4d m) {
        return set(m, false);
    }

    public FrustumIntersection set(Matrix4d m, boolean vulkan) {
        double m00 = m.m00(), m01 = m.m01(), m02 = m.m02(), m03 = m.m03();
        double m10 = m.m10(), m11 = m.m11(), m12 = m.m12(), m13 = m.m13();
        double m20 = m.m20(), m21 = m.m21(), m22 = m.m22(), m23 = m.m23();
        double m30 = m.m30(), m31 = m.m31(), m32 = m.m32(), m33 = m.m33();

        planes[LEFT][0] = m03 + m00;
        planes[LEFT][1] = m13 + m10;
        planes[LEFT][2] = m23 + m20;
        planes[LEFT][3] = m33 + m30;
        normalize(LEFT);

        planes[RIGHT][0] = m03 - m00;
        planes[RIGHT][1] = m13 - m10;
        planes[RIGHT][2] = m23 - m20;
        planes[RIGHT][3] = m33 - m30;
        normalize(RIGHT);

        planes[BOTTOM][0] = m03 + m01;
        planes[BOTTOM][1] = m13 + m11;
        planes[BOTTOM][2] = m23 + m21;
        planes[BOTTOM][3] = m33 + m31;
        normalize(BOTTOM);

        planes[TOP][0] = m03 - m01;
        planes[TOP][1] = m13 - m11;
        planes[TOP][2] = m23 - m21;
        planes[TOP][3] = m33 - m31;
        normalize(TOP);

        if (vulkan) {
            // In Vulkan near plane is z = 0, so it's just the third row of the clip matrix
            planes[NEAR][0] = m02;
            planes[NEAR][1] = m12;
            planes[NEAR][2] = m22;
            planes[NEAR][3] = m32;
        } else {
            // OpenGL near plane is w + z = 0
            planes[NEAR][0] = m03 + m02;
            planes[NEAR][1] = m13 + m12;
            planes[NEAR][2] = m23 + m22;
            planes[NEAR][3] = m33 + m32;
        }
        normalize(NEAR);

        planes[FAR][0] = m03 - m02;
        planes[FAR][1] = m13 - m12;
        planes[FAR][2] = m23 - m22;
        planes[FAR][3] = m33 - m32;
        normalize(FAR);

        return this;
    }

    public FrustumIntersection set(Matrix4f m) {
        return set(m, false);
    }

    public FrustumIntersection set(Matrix4f m, boolean vulkan) {
        double m00 = m.m00(), m01 = m.m01(), m02 = m.m02(), m03 = m.m03();
        double m10 = m.m10(), m11 = m.m11(), m12 = m.m12(), m13 = m.m13();
        double m20 = m.m20(), m21 = m.m21(), m22 = m.m22(), m23 = m.m23();
        double m30 = m.m30(), m31 = m.m31(), m32 = m.m32(), m33 = m.m33();

        planes[LEFT][0] = m03 + m00;
        planes[LEFT][1] = m13 + m10;
        planes[LEFT][2] = m23 + m20;
        planes[LEFT][3] = m33 + m30;
        normalize(LEFT);

        planes[RIGHT][0] = m03 - m00;
        planes[RIGHT][1] = m13 - m10;
        planes[RIGHT][2] = m23 - m20;
        planes[RIGHT][3] = m33 - m30;
        normalize(RIGHT);

        planes[BOTTOM][0] = m03 + m01;
        planes[BOTTOM][1] = m13 + m11;
        planes[BOTTOM][2] = m23 + m21;
        planes[BOTTOM][3] = m33 + m31;
        normalize(BOTTOM);

        planes[TOP][0] = m03 - m01;
        planes[TOP][1] = m13 - m11;
        planes[TOP][2] = m23 - m21;
        planes[TOP][3] = m33 - m31;
        normalize(TOP);

        if (vulkan) {
            planes[NEAR][0] = m02;
            planes[NEAR][1] = m12;
            planes[NEAR][2] = m22;
            planes[NEAR][3] = m32;
        } else {
            planes[NEAR][0] = m03 + m02;
            planes[NEAR][1] = m13 + m12;
            planes[NEAR][2] = m23 + m22;
            planes[NEAR][3] = m33 + m32;
        }
        normalize(NEAR);

        planes[FAR][0] = m03 - m02;
        planes[FAR][1] = m13 - m12;
        planes[FAR][2] = m23 - m22;
        planes[FAR][3] = m33 - m32;
        normalize(FAR);

        return this;
    }

    private void normalize(int index) {
        double invLen = 1.0 / Math.sqrt(
            planes[index][0] * planes[index][0] +
            planes[index][1] * planes[index][1] +
            planes[index][2] * planes[index][2]
        );
        planes[index][0] *= invLen;
        planes[index][1] *= invLen;
        planes[index][2] *= invLen;
        planes[index][3] *= invLen;
    }

    public int testAABB(AABB box) {
        return testAABB(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ);
    }

    public int testAABB(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
        int result = INSIDE;
        for (int i = 0; i < 6; i++) {
            double nx = planes[i][0];
            double ny = planes[i][1];
            double nz = planes[i][2];
            double d = planes[i][3];

            double px = nx >= 0 ? maxX : minX;
            double py = ny >= 0 ? maxY : minY;
            double pz = nz >= 0 ? maxZ : minZ;
            if (nx * px + ny * py + nz * pz + d < 0) {
                return OUTSIDE;
            }

            double npx = nx >= 0 ? minX : maxX;
            double npy = ny >= 0 ? minY : maxY;
            double npz = nz >= 0 ? minZ : maxZ;
            if (nx * npx + ny * npy + nz * npz + d < 0) {
                result = INTERSECT;
            }
        }
        return result;
    }

    public int testSphere(Vector3d center, double radius) {
        return testSphere(center.x(), center.y(), center.z(), radius);
    }

    public int testSphere(Vector3f center, float radius) {
        return testSphere(center.x(), center.y(), center.z(), radius);
    }

    public int testSphere(double cx, double cy, double cz, double radius) {
        int result = INSIDE;
        for (int i = 0; i < 6; i++) {
            double dist = planes[i][0] * cx + planes[i][1] * cy + planes[i][2] * cz + planes[i][3];
            if (dist < -radius) {
                return OUTSIDE;
            }
            if (dist < radius) {
                result = INTERSECT;
            }
        }
        return result;
    }

    public int testPoint(Vector3d point) {
        return testPoint(point.x(), point.y(), point.z());
    }

    public int testPoint(Vector3f point) {
        return testPoint(point.x(), point.y(), point.z());
    }

    public int testPoint(double px, double py, double pz) {
        for (int i = 0; i < 6; i++) {
            double dist = planes[i][0] * px + planes[i][1] * py + planes[i][2] * pz + planes[i][3];
            if (dist < 0) {
                return OUTSIDE;
            }
        }
        return INSIDE;
    }

    public double[] plane(int index) {
        return planes[index];
    }
}
